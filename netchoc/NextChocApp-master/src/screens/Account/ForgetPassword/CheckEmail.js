...
export default function CheckEmail({ route }) {
  ...
  //   }
  // }, [])
  return (<View style={{position:'relative',display:'flex',flex:1}}>
 <LinearGradient colors={['rgb(242,248,231)', '#ffffff', '#ffffff', '#ffffff']} style={{ display:'flex',flex: 1,position:'absolute',width:'100%',height:'100%',top:0,left:0,right:0 }}></LinearGradient>
  <View style={{ marginTop: 60, paddingHorizontal: 12 }}>
    ...
    <Text style={{ fontSize: 32, display: 'flex' }}>Verify Your </Text>
    <Text style={{ fontSize: 32, display: 'flex' }}>Email Address</Text>
    <Text style={{ marginTop: 12, color: 'rgba(0,0,0,0.4)', fontSize: 14 }}>We have just sent a instructions </Text>
    <Text style={{ marginTop: 4, color: 'rgba(0,0,0,0.4)', fontSize: 14 }}>email to {route.params.account}</Text>
    ...
    <View style={{ marginTop: 48, display: 'flex', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, height: 50, width: '100%', borderColor: 'rgba(0,0,0,0.2)', borderWidth: 1, paddingVertical: 4, backgroundColor: 'rgba(0,0,0,0.1)', borderRadius: 24 }}>
      <TextInput style={{ flex: 1, paddingVertical: 8 }} maxLength={6} value={code} onChangeText={getCode} placeholder="code"  ></TextInput>
      {
        <Text style={{ width: 120, textAlign: 'center', fontSize: 18 }}>
          Resend in : {time}
        </Text>
      }
    </View>
