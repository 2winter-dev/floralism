...
export default function Verify({ route }) {
  ...
  <View style={{ marginTop: Dimensions.get("screen").height * 0.06, paddingHorizontal: 12 }}>
    ...
    <Text style={{ fontSize: 32, display: 'flex' }}>phone number</Text>
    <Text style={{ marginTop: 12, color: 'rgba(0,0,0,0.4)', fontSize: 18 }}>Enter the 6-Digit code sent  </Text>
    <Text style={{ marginTop: 12, color: 'rgba(0,0,0,0.4)', fontSize: 18 }}>to you on {"+" + route.params.area_code + route.params.account}</Text>
    <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
      <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginTop: 24 }}>
        <Text style={{ fontSize: 14, marginRight: 8,color:config.text_color_1 }}>Didn't receive?</Text>
