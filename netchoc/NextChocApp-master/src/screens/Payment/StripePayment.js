...
import { View, ActivityIndicator, Pressable, Text, Keyboard, Linking, ScrollView, Dimensions, Image } from "react-native";
...
export default function StripePayment({ publishableKey = "", client_secret = "", data, payFailed, payFinish, payment_amount }) {
  ////console.log("===");
  ////console.log(data);
  ...
  return (<View style={{
    ...
    {/* <ScrollView style={{ marginHorizontal: 16, maxHeight: Dimensions.get("screen").height * 0.3 }}>
      <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
        <View>
          <Text style={{ fontSize: 18, fontWeight: 500 }}>{data?.merchant?.merchant_name}</Text>
          <Text style={{ fontSize: 12, marginTop: 8 }}>{data?.merchant?.merchant_category_name}</Text>
        </View>
      </View>
      <View style={{borderBottomColor:'rgba(0,0,0,0.4)',
      borderBottomWidth:0.5}}>
      {
         data?.goods?.length&&data.goods.map((item,index)=>{
          return (<View key={index} style={{paddingVertical:12,display:'flex',flexDirection:'row',alignItems:'center'}}>
              <Image source={{uri:item.cover_image}} style={{width:60,height:60,borderRadius:12,marginRight:8}} />
              <View style={{flex:1,marginRight:12}}>
                 <Text numberOfLines={1} style={{fontSize:16}}>{item.goods_name}</Text>
                 <Text numberOfLines={1} style={{fontSize:12,color:'rgba(0,0,0,0.4)'}}>{item.description}</Text>
                 <Text style={{fontSize:12}}>×{item.count}</Text>
              </View>
              <Text>{config.current}{(item.count * item.discount_price).toFixed(2)}</Text>
          </View>)
         })
      }
      </View>
       <View style={{display:'flex',flex:1,flexDirection:'row',justifyContent:'flex-end',alignItems:'baseline',marginTop:12}}>
          <Text style={{fontSize:12}}>Total: {config.current}</Text>
          <Text style={{fontSize:16,fontWeight:500}}>{payment_amount}</Text>
        </View>
    </ScrollView> */}
