import { View, Text, Pressable, TextInput, Dimensions, Keyboard, ActivityIndicator, Alert } from 'react-native'
...
export default function ProfileSetting(props) {
    ...
    useEffect(()=>{
       if(!token)  navigation.navigate("SignIn.LoginWithPhone");
    },[])
    ...
    return (<View style={{ paddingHorizontal: 16 }}>
        {!loginmess && <>
            ...
            <Pressable onPress={() => {
                if (!token) navigation.navigate("SignIn.LoginWithPhone");
                navigation.navigate('Account.login.setting')
            }} style={{ marginTop: 36 }}>
            ...
            {
                <Pressable onPress={() => {
                    Alert.alert("Notice", "Do you want to delete this account?", [
                        {
                            text: "confirm",
                            onPress: () => {
                                navigation.navigate("SignIn.LoginWithPhone");
                                Toast.show("We will process your request within 48 hours");
                                logout();
                ...
                }} style={{ marginHorizontal: 24, marginTop: 24, borderRadius: 24, display: 'flex', backgroundColor: 'tomato', alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ color: 'white', paddingVertical: 12, fontSize: 16 }}>Delete Account</Text>
                </Pressable>
            }
