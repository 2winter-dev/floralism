    ...
    <div className={`${header.header_background}`}>
      <div className={`${header.header_layout}`}>
        ...
        <div className={`${header.header_center}`}>
          {props?.list?.length &&
            props.list.map((item, index) => {
              ...
              return (
                <div key={item.id} className={`${header.header_center_label}`}>
                  <div
                    onClick={() =>
                      router.push(
                        !!item?.get_child.length &&
                          `/category/${item.get_child[0].id}`
                      )
                    }
                    className={`${header.header_center_parent}`}
                  >
                    {item.categoryname}
                    {!!item.get_child?.length && (
                      <span
                        className={`iconfont ${header.dropdown_btn}`}
                        style={{ fontSize: 10 }}
                      >
                        &#xe645;
                      </span>
                  ...
                  {!!item.get_child?.length && (
                    <div className={`${header.header_center_dropdown}`}>
                      {item.get_child.map((it, ii) => {
                        return (
                          <div key={index.toString() + ii.toString()}>
        ...
        <div className={`${header.header_right}`} style={{ marginLeft: 10 }}>
          <div
            onClick={() => {
              Cookies.get("token") ? router.push(`/User`) : props.login();
            }}
            style={{ display: "flex", alignItems: "center", cursor: "pointer" }}
          >
            <span style={{}} className={`iconfont ${header.mine_icon}`}>
              &#xe70e;
            </span>
      ...
      <div className={`${header.header_menu}`}>
        <div
          className={`${header.header_cate_layout}`}
